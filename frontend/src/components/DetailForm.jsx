...
export default function DetailForm() {
  ...
  const fetchTypes = () => {
    ...
    fetch(
      `${S_URL}/api/details/types?shape=${encodeURIComponent(formData.shape)}`,
      {
        headers: {
          Authorization: `Bearer ${localStorage.getItem("token")}`,
        },
  ...
  useEffect(() => {
    if (formData.shape) {
      fetch(
        `${S_URL}/api/detail_attributes?shape=${encodeURIComponent(
          formData.shape
        )}`,
        {
          headers: {
  ...
  const handleSubmit = () => {
    ...
    const areMainFieldsFilled = requiredFields.every(
      (val) => val && val.trim() !== ""
    );
    ...
    const detailPayload = {
      ...
      type: formData.type,
      attributes: Object.values(attributeValues).map(
        ({ attributeId, value }) => ({
          attributeId,
          value,
        })
  ...
  if (mode === "typeShape") {
    return (
      <div className="form">
        ...
        <label>Форма</label>
        <input
          value={shapeInputForType}
          onChange={(e) => setShapeInputForType(e.target.value)}
        />
  ...
  if (mode === "attrShape") {
    return (
      <div className="form">
        ...
        <input
          value={attrShape}
          onChange={(e) => setAttrShape(e.target.value)}
        />
  ...
  if (mode === "myDetails") return <MyDetails />;
  return (
    <div className="form">
      <h3>Обрати деталь</h3>
      ...
      <button onClick={() => setMode("myDetails")}>Мої деталі</button>
    </div>
  );
}
